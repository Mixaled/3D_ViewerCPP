...
Mesh::Mesh(const std::string &filename) : m_num_vertices(0), m_num_faces(0) {
  m_bmin = Eigen::Vector3f(std::numeric_limits<float>::max(),
                           std::numeric_limits<float>::max(),
                           std::numeric_limits<float>::max());
  m_bmax = Eigen::Vector3f(-std::numeric_limits<float>::max(),
                           -std::numeric_limits<float>::max(),
                           -std::numeric_limits<float>::max());
...
bool Mesh::load_obj(const std::string &filename) {
  ...
  bool load = tinyobj::LoadObj(&attrib, &m_shapes, &m_materials, &err,
                               filename.c_str());
  ...
  for (size_t s = 0; s < m_shapes.size(); s++) {
    for (size_t f = 0; f < m_shapes[s].mesh.indices.size() / 3; f++) {
      ...
      m_dist_max = std::max((m_bmax - m_mesh_center).norm(),
                            (m_bmax - m_mesh_center).norm());
...
unsigned int Mesh::get_number_of_face() { return m_num_faces; }

const Eigen::Vector3f Mesh::get_mesh_center() { return m_mesh_center; }

const Eigen::MatrixXf *Mesh::get_points() { return &m_points; }

const MatrixXu *Mesh::get_indices() { return &m_indices; }

const Eigen::MatrixXf *Mesh::get_normals() { return &m_normals; }

float Mesh::get_dist_max() { return m_dist_max; }

unsigned int Mesh::get_number_of_vertices() { return m_num_vertices; }
